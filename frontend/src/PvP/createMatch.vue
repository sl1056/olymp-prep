      ...
      <div class="setting-section">
        ...
        <div class="question-count-options">
          <button
            ...
            :key="count"
            :class="['question-count-btn', { selected: currentQuestionCount === count }]"
...
export default {
  ...
  computed: {
    // Можно ли начинать
    canStart() {
      return !this.isStarting && this.currentSubject && this.currentDifficulty && this.currentQuestionCount > 0
    },
    ...
    debugInfo() {
      return {
        ...
        questionCount: this.currentQuestionCount,
  ...
  methods: {
    getSubjectForApi() {
      ...
      return map[this.currentSubject] || this.currentSubject;
    },
    ...
    setQuestionCount(count) {
      this.currentQuestionCount = count
    },
    ...
    async createMatch() {
      ...
      try {
        ...
        const subject = this.getSubjectForApi();
        const payload = {
          difficulty: this.currentDifficulty,
        ...
        this.matchCode = response.data.match_code || String(response.data.match_id);
        ...
        localStorage.setItem('currentMatchQuestionsCount', String(this.currentQuestionCount));
  ...
  mounted() {
    ...
    if (saved) {
      try {
        ...
        this.currentQuestionCount = Number(parsed.questionCount) || 5
      } catch (e) {
...
.question-count-btn.selected {
  background: linear-gradient(135deg, #ebf8ff, #e6fffa);
  border-color: #3182ce;
  box-shadow: 0 5px 15px rgba(49, 130, 206, 0.12);
}
